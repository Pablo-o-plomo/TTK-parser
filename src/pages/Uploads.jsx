...
const UPLOAD_STATUS = { uploaded:'Загружено', parsing:'Распознаётся', parsed:'Распознано', needs_mapping:'Нужно сопоставить колонки', imported:'Импортировано', error:'Ошибка' }
...
export default function Uploads({ uploads = [], addUpload }) {
  ...
  function submit(e) {
    ...
    const record = addUpload({ mode, restaurant, uploadType, comment, file, mapping: mode === 'table' ? columns : [], previewRows: mode === 'table' ? Array.from({ length: 20 }, (_, i) => `Строка ${i + 1}: preview ожидает парсер`) : [] })
    setLast(record)
    setFile(null)
    setComment('')
  }
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
      <div style={{ background:'#fff', border:'1px solid #e8ecf0', borderRadius:14, padding:18 }}>
        ...
        <div style={{ color:'#64748b', fontSize:13 }}>Ручная загрузка Excel/CSV или PDF. Пока файлы сохраняются в localStorage и ждут будущего парсера.</div>
      </div>
      <form onSubmit={submit} style={{ background:'#fff', border:'1px solid #e8ecf0', borderRadius:14, padding:18, display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(190px,1fr))', gap:12, alignItems:'end' }}>
      ...
      {last && <div style={{ background:'#f0fdf4', border:'1px solid #bbf7d0', borderRadius:14, padding:14, color:'#166534' }}>Файл “{last.file.name}” загружен. Статус: {UPLOAD_STATUS[last.status]}.</div>}

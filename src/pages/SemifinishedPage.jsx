...
import { createEmptySemifinished, SEMIFINISHED_STATUS_LABELS } from '../hooks/useSemifinished.js'
...
const TEXTAREA = { width:'100%', boxSizing:'border-box', minHeight:90, border:'1.5px solid #e5e7eb', borderRadius:12, padding:12, fontSize:13, outline:'none', resize:'vertical', fontFamily:'inherit' }
...
function SemiForm({ initial, products = [], onSave, onCancel }) {
  ...
  const productOptions = useMemo(() => {
    ...
    return products
      .filter(item => [item.name, item.category, item.categoryPath].join(' ').toLowerCase().includes(needle))
      .slice(0, 12)
  ...
  function addProductToComposition(product) {
    const line = `${product.name}${product.unit ? ` — ${product.unit}` : ''}`
    setForm(current => ({
      ...current,
      composition: current.composition ? `${current.composition}\n${line}` : line,
    }))
    setProductQuery('')
  }
  ...
  return (
    <form onSubmit={e => { e.preventDefault(); onSave(form) }} style={{ ...SECTION, display:'flex', flexDirection:'column', gap:14 }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', gap:12 }}>
        <div>
          <h2 style={{ margin:'0 0 4px' }}>{initial ? 'Редактировать полуфабрикат' : 'Добавить полуфабрикат'}</h2>
          <div style={{ color:'#64748b', fontSize:13 }}>Полуфабрикат — это заготовка, соус, база, бульон или подготовленный компонент.</div>
        </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(210px,1fr))', gap:12 }}>
        ...
        <Field label="Статус">
          <select value={form.status || 'active'} onChange={e => update('status', e.target.value)} style={{ ...SEL_ST, width:'100%' }}>
      ...
      <section style={{ border:'1px solid #e5e7eb', borderRadius:14, padding:12, background:'#f8fafc' }}>
        <div style={{ fontSize:12, fontWeight:900, color:'#475569', marginBottom:8 }}>Добавить товар в состав</div>
        <input value={productQuery} onChange={e => setProductQuery(e.target.value)} placeholder="Начните вводить товар: соль, сливки, креветка..." style={INPUT} />
        {productQuery && (
          <div style={{ display:'flex', gap:6, flexWrap:'wrap', marginTop:8 }}>
            {productOptions.map(product => (
              <button key={product.id} type="button" onClick={() => addProductToComposition(product)} style={SEL_ST}>
                + {product.name}
              </button>
            ))}
          </div>
      ...
      <Field label="Состав">
        <textarea value={form.composition || ''} onChange={e => update('composition', e.target.value)} style={TEXTAREA} />
      </Field>

      <Field label="Способ приготовления">
        <textarea value={form.cookingMethod || ''} onChange={e => update('cookingMethod', e.target.value)} style={TEXTAREA} />
      </Field>

      <Field label="Описание / комментарий">
        <textarea value={form.description || ''} onChange={e => update('description', e.target.value)} style={TEXTAREA} />
      </Field>
    </form>
  )
}

export default function SemifinishedPage({ items = [], products = [], onSave, onDelete, onImport }) {
  ...
  async function handleImport(file) {
    ...
    try {
      ...
      setMessage(`Импортировано полуфабрикатов: ${count}`)
    } catch {
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
      <section style={{ ...SECTION, display:'flex', justifyContent:'space-between', alignItems:'center', gap:12 }}>
        <div>
          ...
          <div style={{ color:'#64748b', fontSize:14 }}>Отдельный справочник заготовок. Используется в эталонных ТТК.</div>
        </div>
        <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
          ...
          <button type="button" onClick={() => downloadJson('klevo_semifinished.json', items)} style={SEL_ST}>Экспорт JSON</button>
          <button type="button" onClick={startCreate} style={PRIMARY}>Добавить П/Ф</button>
        </div>
      </section>
      ...
      <section style={{ ...SECTION, display:'grid', gridTemplateColumns:'260px 1fr', gap:14 }}>
        <aside style={{ borderRight:'1px solid #e5e7eb', paddingRight:14 }}>
          ...
          {groups.map(group => (
            <button
              key={`${group.depth}-${group.name}`}
              type="button"
              onClick={() => setSelectedGroup(group.name)}
              style={{
                ...GROUP_BTN,
                paddingLeft:10 + group.depth * 16,
        ...
        <div>
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Поиск полуфабриката" style={INPUT} />
        </div>
      </section>
      ...
      <section style={SECTION}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
          <h2 style={{ margin:0 }}>Полуфабрикаты ({filtered.length})</h2>
          <div style={{ color:'#64748b', fontSize:12 }}>localStorage key: <b>klevo_semifinished</b></div>
        </div>

        {filtered.length === 0 ? (
          <div style={{ textAlign:'center', color:'#94a3b8', padding:34 }}>
            Полуфабрикаты не найдены. Импортируйте CSV/JSON списком или добавьте вручную.
          </div>
        ) : (
          <div style={{ overflowX:'auto' }}>
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
              <thead>
                <tr>
                  {['Название', 'Выход', 'Категория', 'Древо группы', 'Состав', 'Технология', 'Статус', ''].map(header => (
              ...
              <tbody>
                {filtered.map(item => (
                  <tr key={item.id}>
                    ...
                    <td style={TD}>{item.output || '—'}</td>
                    <td style={TD}>{item.category || '—'}</td>
                    <td style={TD}>{item.categoryPath || '—'}</td>
                    <td style={TD}>{item.composition || '—'}</td>
                    <td style={TD}>{item.cookingMethod || item.description || '—'}</td>
                    <td style={TD}>{SEMIFINISHED_STATUS_LABELS[item.status] || item.status}</td>
